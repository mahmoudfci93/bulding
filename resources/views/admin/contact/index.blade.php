    ...
    <script type="text/javascript">
            ...
            $('#data thead th').each( function () {
                if($(this).index()  < 4 ){
                    ...
                    $(this).html( '<select><option value="0"> رسالة قديمة </option><option value="1">  رسالة جديدة </option></select>' );
                }
                else if($(this).index() == 5){
                    $(this).html( '<select>' +
                         @foreach(contact() as $key=>$contacts)
                            '<option value="{{$key}}">{{$contacts}}</option>'+
                         @endforeach
                        '</select>' );
                }
        ...
        var table = $('#data').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ url('/admin-panel/contact/data') }}',
            columns: [
                {data: 'id', name: 'id'},
                {data: 'name', name: 'name'},
